...
def load_and_prepare_targets(file_path: Path) -> dict[str, TargetInfo]:
    ...
    Args:
        file_path: Path to either:
            - A CSV file with "Structure ID" and "SMILES" columns
            - A JSON file with format {"target_id_1": "SMILES_1", ...}
            JSON files may be gzipped (.json.gz)

    Returns:
        A dictionary mapping target IDs to validated TargetInfo objects.

    Raises:
        UrsaException: If a target SMILES is invalid or file format unsupported.
    """
    logger.info(f"Loading and preparing targets from {file_path}...")

    # Load raw targets based on file extension
    try:
